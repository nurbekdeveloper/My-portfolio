...
const Works = () => {
  return (
    <div className='bg-[#edf2f8]'>
      <div className='font-bold flex gap-3  py-5 text-[35px] md:text-50px  my-10 md:my-16 justify-center'>
          <span>My</span>
      ...
      <div className='flex justify-center flex-col gap-3  items-center'>
        {worksDB.map((item)=>(
            ...
            <div className=' flex flex-col gap-3  md:gap-5'>
              <h1 className='font-bold text-[15px]'>{item.title}</h1>
              <p className='text-[#5e6064]'>{item.date}</p>
              <p   className='text-[#5e6064]'>{item.description}</p>
              <b>Stack:</b>
              <p>{item.stack}</p>
